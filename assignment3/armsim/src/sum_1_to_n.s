...
@latency of the instructions is in cycles and energy is in pJ (pico joules), frequency of the processor = 1GHz
	...
	mov r1, #100		@Moves constant 1000 to register r1, latency = 1, energy = 100 
	mul r1,r1,#10
        mov r2, #1
	ldr r3, =AA		@Loads address of label AA in register r3, latency = 1, energy = 110 		

StoreIntegers:
	str r2, [r3,#0]		@ Stores the value of register r2 to address stored in register r3, latency = 20, energy = 2000
...
mov r4, #0
	ldr r3, =AA 
LoadAddIntegers:
	ldr r2, [r3,#0]     	@ Loads integers from address stored in register r3 to register r2, latency = 20, energy = 2000
